...
namespace ProceduralTerrainGeneration
{
    class SimplexNoise
    {
        ...
        public SimplexNoise(int width, int height, float scale, int seed)
        {            
            _perm = new byte[512];            
            var random = new Random(seed);
            random.NextBytes(_perm);            

            HeightmapAsArray = new float[width, height];
            for (var i = 0; i < width; i++)
                for (var j = 0; j < height; j++) {                    
                    HeightmapAsArray[i, j] = Generate(i * scale, j * scale) * 0.5f + 0.5f;            
                }
        }
